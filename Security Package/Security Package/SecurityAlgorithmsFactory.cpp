...
Security_Package* SecurityAlgorithmsFactory::createAlgorithm(string p_AlgoName)
{
	if(p_AlgoName == "AES")
		...
		if(p_AlgoName == "DES")
			...
			if(p_AlgoName == "AutoKey")
				return new AutoKey();
			else
				if(p_AlgoName == "HiLL_Cipher")
					return new Hill_Cipher();
				else
					if(p_AlgoName == "RailFence")
						return new RailFence();
					else
						if(p_AlgoName == "Playfair_Cipher")
							return new Playfair_Cipher;
						else
							if(p_AlgoName == "Monoalphabetic_Cipher")
								return new Monoalphabetic_Cipher();
							else
								if(p_AlgoName == "Repeatingkey")
									return new RepeatingKey();
								else
									if(p_AlgoName == "Columnar")
										return new Columnar();
									else
										if(p_AlgoName == "Ceaser_Cipher")
